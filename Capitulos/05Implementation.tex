...
La tabla \var{Scans} (tabla~\ref{tab:tableScans}) hace referencia a los datos de sensado del entorno. La clave primaria está formada por el par (\var{id\_node}, \var{id\_scan}), de tal forma que no se permiten valores repetidos de este par. Vinculamos esta tabla con la tabla \var{Nodes} gracias a la clave foránea: \var{id\_node}, que hace referencia al campo \var{id\_node} de \var{Nodes} de tal forma que no se permite introducir datos del entorno de nodos que no existan y borrar nodos en la tabla \var{Nodes} si existen datos en la tabla \var{Scans} para no dejar huérfano el dato. La manera de introducir nuevos valores implica una consulta anterior para averiguar cuál es el número de \var{id\_scan} que corresponde al nodo al que queremos vincular el dato. Este proceso está encapsulado en el método que se publica en la capa de datos siendo transparente al servicio.
...
Estos mensajes son resultado en última instancia del submódulo
\var{Gestor de Inter\-faces} que es el punto de salida del módulo cognitivo
...
\begin{itemize}
	\item La clase \class{BluetoothController} manejadora del establecimiento y mantenimiento de las conexiones sobre un canal \ac{RFCOMM}.
...
\begin{itemize}
 ...
 \item Un \integer{} con el identificador el nodo. Este identificador nos lo facilita el servicio por ejemplo en sus mensajes de información (ver secciones~\ref{subsec:serviceInfo} y~\ref{subsec:serviceInfoImpl}).
\end{itemize}

\messageTableInit{\appToServ}{12 = \footnotesize{CONNECT\_TO\\\_VIA\_BT}}{\nc{}}{\nc{}}{nodo al que conectarse}{\nc{}}
\messageTableEnd{Mensaje para conectarse manualmente a un nodo en \bt{} }{connectToBT}
...
La estructura del mensaje de la tabla~\ref{tab:wa_reply} permite a un nodo normal guardar la respuesta que suministrará en caso de ser requerida.
...
En el campo \obj{} codificamos la respuesta como una \String{}.\\
...
Cuando el servicio ha conseguido la respuesta envía a todas las aplicaciones pues no tiene forma de deducir por el contexto al ser procesos no correlados, aunque aquí lo estamos viendo como uno solo. La respuesta viene codificada en el campo \obj{} del mensaje~\ref{tab:wa_ask_response}.
...
que son usados por uno o más procesos.\\[30pt]
...
\protobufTableInitLong{Estructura mensaje informe de la actualización de parámetros de un nodo}
	...
	\begin{description}
		...
		\protobufItem{Mensaje: \texttt{Topology}}{Un contenedor que contiene al mensaje del tipo que se indicó en el campo ``\texttt{signal}'':}
			...
			\end{description}
	\end{description}
\protobufTableEnd{Campos del mensaje informe de actualización de parámetros de un nodo hacia el resto, estructura siguiendo la figura~\ref{fig:messageProto}}{updateParams}
...
Ambos controladores entregan los datos leídos al servicio (ver tabla~\ref{tab:messageReadController}) para que interprete y actúe presentando y/o reenviando estos datos. Para ello el \var{Gestor de interfaces} se ayuda del campo ``\texttt{id\_app}'' o ``código de aplicación'' para discernir qué aplicación está interesada en los datos recibidos y del campo ``\texttt{from}'' para averiguar el origen del mensaje, ambos campos están en los \textit{bytes} leídos de la red.
...
El mensaje que devolvemos de vuelta al nodo originario confirmando la entrega del mensaje que acabamos de reenviar se estructura de la siguiente manera:
  ...
  \protobufItem{Mensaje: \texttt{Response}}{Un contenedor que contiene al mensaje del tipo que se indicó en el campo ``\texttt{type}'':}
	\begin{description}
		...
		\protobufItem{Mensaje: \texttt{SCAN}}{Un contenedor que contiene al mensaje del tipo que se indicó en el campo ``\texttt{what}'':}
			\begin{description}
				\protobufItem{\texttt{priority}}{El enumerado ``Priority'' con la urgencia del sensado, toma dos valores {\small Priority.TASK} para sensados normales y {\small Priority.URGENT} para marcar que la información del entorno es sensible.}  
				...
				\protobufItem{\texttt{throughput}}{Un \double{} con el resultado promedio móvil del intervalo medido en segundos entre la llegada de mensajes de datos por parte de las aplicaciones.}
			\end{description}
	\end{description}
		...
		\protobufItem{Mensaje: \texttt{Topology}}{Un contenedor que contiene al mensaje del tipo que se indicó en el campo ``\texttt{signal}'':}
			\begin{description}
				\protobufItem{\texttt{change}}{Valor \bsq{ALL\_END\_SWITCH} del enumerado ``TopologyChange''.}
					\begin{description}
						\protobufItem{Mensaje: \texttt{Node}}{Mensaje con los cambios que sirven para representar a nodo (ver tabla~\ref{itm:infoNode}). Tendremos tantas representaciones como nodos haya en la red.}
					\end{description}				
			\end{description}
	\end{description}
...
\protobufTableInit[!h]{Estructura mensaje petición información (\textit{test})}   
  ...
  \protobufItem{Mensaje: \texttt{Request}}{Un contenedor que contiene al mensaje del tipo que se indicó en el campo ``\texttt{type}'':}
	...
	\end{description}
\protobufTableEnd{Campos del mensaje petición de información (\textit{test})}{wa_ask_protoMessage}
  ...
  \protobufItem{Mensaje: \texttt{Response}}{Un contenedor que contiene al mensaje del tipo que se indicó en el campo ``\texttt{type}'':}
	\begin{description}
		...
		\protobufItem{Mensaje: \texttt{Test}}{Un contenedor que contiene al mensaje del tipo que se indicó en el campo ``\texttt{what}'':}
			...
			\end{description}
	\end{description}
\protobufTableEnd{Campos del mensaje entrega de información (\textit{test})}{wa_ask_reply_protoMessage}
