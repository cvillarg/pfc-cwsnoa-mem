...
hacer una petición de union (\textit{bind}) llamando al método \textit{bindService()} con un \textit{Intent} a la clase:
\begin{verbatim}
...
%XXX-jblesa quita handshaking y ponlo en español.

Como hemos visto ya en la \ac{API} (ver sección \ref{sec:api}), tras
la petición de \textit{bind}, que inicializará el servicio si no
...
%XXX-jblesa pon bind en español
...
%XXX-jblesa No funciona la referencia sec:analisisWifi
...
%XXX-jblesa A quien te refieres con ``que nosotros procesamos'' en el parrafo anterior?
...
%XXX-jblesa quita unbind y ponlo en español.
...
%Nodo normal y Nodo coordinador.
...
Las acciones a ejecutar por los nodos que se quedan en la red son:
\paragraph{Normal} Si somos un nodo normal, el evento elevado hasta el
...
toda la red, por lo tanto marcamos a todos los nodos como inactivos
...
%XXX-jblesa. En el parrafo anterior dos veces por tanto seguidas.
...
%XXX-jblesa. Este parrafo anterior no tiene sentido. Escribelo otra vez
...
%XXX-jblesa quita forward y ponlo en español
...
conocer cuáles son los siguientes nodos a los que enviar el mensaje
(prestando especial atención a que si es un mensaje dirigido a todos,
no volver a enviarlo al nodo originario de la comunicación). el
mensaje a enviar que es el mismo que acabamos de recibir (sin hacerle
ningún cambio) y por último el parámetro que antes obviamos.

%XXX-jblesa La ultima frase no se entiende... ``el mensaje...''
...
contextual para evitar bloqueos porque algún no responda. A partir de
...
aplicación permite gracias a un \textit{shortcut}
\textit{(workaround)} el forzado para el encolamiento. Al pulsar el
botón diseñado a tal efecto, se nos pregunta a través de una cadena de
diálogos el mensaje a encolar y a que destinatarios debe dirigirse.

%XXX-jblesa workaround y shortcut en español
