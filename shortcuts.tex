...
un conjunto de dos \textit{arrays} ligados, uno con los identificadores de los nodos y otro con los nombres de \'{e}stos. De tal forma que el par (identificador, nombre) comparta la misma posici\'{o}n en ambos \textit{arrays}. Se recuperan con las claves ``\texttt{nodeIdsList}'' y ``\texttt{nodeNamesList}'' respectivamente.
}
...
\providecommand{\messageTableExtraControllerSpecified}[3]{%
	...
	\multicolumn{5}{l}{\textit{Extras}, controlador: {#1}}\\%
	\hline%
	\textit{key} & \multicolumn{4}{l|}{\textit{value}}\\%
	\hline
...
\providecommand{\protobufTableInitLong}[2][h]{%
\begin{table}[#1]%
	\begin{center}%
		\begin{tabular}{c}%
			\textbf{#2}\\ \hline%
...
\begin{center}%
	\begin{tabular}{c r @{\hskip 7pt} l}%
		\hline%
		\multicolumn{3}{|c|}{Tabla: \texttt{#1}}\\ \hline% 
		\noalign{\vskip 6pt}%
}%
